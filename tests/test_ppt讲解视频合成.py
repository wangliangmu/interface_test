...
class TestPpt讲解视频合成(BaseTest):
    ...
    def test_step_02_post_14_compose(self):
        ...
        try:
            ...
            logger.info(
                f"创建PPT视频合成任务成功，任务ID: {self.context['compose_id']}"
            )
        except Exception:
    ...
    def test_step_04_get_pptvideo_taskspage_size1page1typeppttask_typev(self):
        ...
        try:
            ...
            assert (
                status == 2
            ), f"期望状态 2 (成功)，实际状态 '{status}': {response.text[:200]}"
            assert extract_json_path(
                response.json(), "$.data.list[0].id"
            ) == self.context.get(
                "compose_id"
            ), f"Expected list[0].id={{compose_id}}, got {response.text[:200]}"
        except Exception as e:
